...
2. New password root: aluno <Enter>
3. Re-enter new password root: aluno <Enter>
...
3. New password: aluno <Enter>
4. Re-enter new password: aluno <Enter>
...
SHOW DATABASES;
#
# Criando o nosso Banco de Dados aluno no SGBD do MySQL ou MariaDB
...
# Primeiro criar o usuário: CREATE USER 'aluno'@'localhost' IDENTIFIED WITH mysql_native_password BY 'aluno';
# Segundo aplicar as permissões: GRANT ALL PRIVILEGES ON *.* TO 'aluno'@'localhost';
# Terceiro aplicar todas as mudanças: FLUSH PRIVILEGES;
GRANT ALL ON *.* TO root@'%' IDENTIFIED BY 'aluno'
#
...
#
# Aplicando as permissões de acesso ao Banco de Dados aluno no SGBD do MySQL ou MariaDB
# (opções do comando GRANT: grant (permissão), usage (uso em banco ou tabela), on (em ou na | banco ou tabela), *.* (todos os bancos/tabelas))
# (opções do comando GRANT: to (para), 'aluno' (usuário), identified by (identificado por - senha do usuário))
# (opções do comando GRANT: all (todos privilégios), privileges (privilégios), on (em ou na | banco ou tabela), aluno.* (banco/tabelas), to (para) 'aluno' (usuário))
GRANT USAGE ON *.* TO 'aluno' IDENTIFIED BY 'aluno';
GRANT ALL PRIVILEGES ON aluno.* TO 'aluno';
FLUSH PRIVILEGES;
EXIT
#
# Acessando o SGBD do MySQL ou MariaDB com o usuário aluno
# (opções do comando mysql: -u user | -p password)
mysql -u aluno -p
#
# Utilizando o Banco e Dados aluno no SGBD do MySQL ou MariaDB
SHOW DATABASES;
USE aluno;
#
...
INSERT INTO alunos VALUES ('000001', 'Robson aluno', 'Guarulhos');
